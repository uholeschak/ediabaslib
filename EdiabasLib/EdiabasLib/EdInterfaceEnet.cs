    ...
    public class EdInterfaceEnet : EdInterfaceBase
    {
        ...
        public override byte[] KeyBytes
        {
            get
            {
                if (SharedDataActive.DiagRplus)
                {
                    EdiabasNet.ErrorCodes errorCodeNmt = NmtGetKeyBytes(out byte[] keyBytes);
                    if (errorCodeNmt != EdiabasNet.ErrorCodes.EDIABAS_ERR_NONE)
                    {
                        EdiabasProtected?.SetError(errorCodeNmt);
                        return ByteArray0;
        ...
        public override Int64 BatteryVoltage
        {
            get
            {
                ...
                if (SharedDataActive.DiagRplus)
                {
                    EdiabasNet.ErrorCodes errorCodeNmt = NmtGetPowerSupply(out Int32 voltage);
                    if (errorCodeNmt != EdiabasNet.ErrorCodes.EDIABAS_ERR_NONE)
                    {
                        EdiabasProtected?.SetError(errorCodeNmt);
            ...
            try
            {
                ...
                if (SharedDataActive.EnetHostConn == null)
                {
                    if (RemoteHostProtected.StartsWith(AutoIp, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, string.Format("Received: Connection={0}", SharedDataActive.EnetHostConn));
        ...
        public override bool InterfaceReset()
        {
            ...
            if (SharedDataActive.DiagRplus)
            {
                EdiabasNet.ErrorCodes errorCode = NmtReset();
                if (errorCode != EdiabasNet.ErrorCodes.EDIABAS_ERR_NONE)
                {
                    EdiabasProtected?.SetError(errorCode);
                }
        ...
        public override bool InterfaceBoot()
        {
            ...
            if (SharedDataActive.DiagRplus)
            {
                EdiabasNet.ErrorCodes errorCode = NmtWarmStart();
                if (errorCode != EdiabasNet.ErrorCodes.EDIABAS_ERR_NONE)
                {
                    EdiabasProtected?.SetError(errorCode);
                }
                ...
                for (;;)
                {
                                            ...
                                            if (broadcastAddressName != null && !broadcastAddressName.StartsWith("127."))
                                            {
                                                try
                                                {
                                                    ...
                                                    if (protocolHsfz)
                                                    {
                                                        ...
                                                        TcpClientWithTimeout.ExecuteNetworkCommand((string bindIpAddress) =>
                                                        {
                                                            EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, string.Format("Ident bind IP={0}", bindIpAddress ?? string.Empty));
                                                    ...
                                                    if (protocolDoIp)
                                                    {
                                                        ...
                                                        TcpClientWithTimeout.ExecuteNetworkCommand((string bindIpAddress) =>
                                                        {
                                                            EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, string.Format("DoIp ident bind IP={0}", bindIpAddress ?? string.Empty));
                                                    ...
                                                    TcpClientWithTimeout.ExecuteNetworkCommand((string bindIpAddress) =>
                                                    {
                                                        EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, string.Format("SrvLoc bind IP={0}", bindIpAddress ?? string.Empty));
                    ...
                    else
                    {
                        try
                        {
                            ...
                            TcpClientWithTimeout.ExecuteNetworkCommand((string bindIpAddress) =>
                            {
                                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, string.Format("SvrLoc bind IP={0}", bindIpAddress ?? string.Empty));
                                UdpSocket.SendTo(UdpSvrLocReq, ipUdpSvrLoc);
                            }, ipUdpSvrLoc.Address, SharedDataActive.NetworkData);
        ...
        protected EdiabasNet.ErrorCodes NmtSwitchSiRelais(UInt32 time)
        {
            if (SharedDataActive.ReconnectRequired)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtSwitchSiRelais Reconnecting");
                InterfaceDisconnect(true);
                if (!InterfaceConnect(true))
                {
                    EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtSwitchSiRelais Reconnect failed");
                    SharedDataActive.ReconnectRequired = true;
                    return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
                }
            }
            ...
            timeBytes[0] = (byte)(time & 0xFF);
            timeBytes[1] = (byte)((time >> 8) & 0xFF);

            List<NmpParameter> paramListSend = new List<NmpParameter>()
            {
                new NmpParameter(timeBytes),
            };

            List<NmpParameter> paramListRec = TransNmpParameters(timeout, SharedDataActive.NmpChannel, EdiabasNet.IfhCommands.IfhSwitchSiRelais, paramListSend);
            if (paramListRec == null || paramListRec.Count < 4)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "*** NMT switch SI relais failed");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }
            ...
            if (errorCode == null)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT switch SI relais invalid parameters");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "NMT switch SI relais result: {0}", errorCode.Value);
            return errorCode.Value;
        }
        ...
        protected EdiabasNet.ErrorCodes NmtWarmStart()
        {
            if (SharedDataActive.ReconnectRequired)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtWarmStart Reconnecting");
                InterfaceDisconnect(true);
                if (!InterfaceConnect(true))
                {
                    EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtWarmStart Reconnect failed");
                    SharedDataActive.ReconnectRequired = true;
                    return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
                }
            }
            ...
            List<NmpParameter> paramListSend = new List<NmpParameter>()
            {
                new NmpParameter(commandBytes),
            };

            List<NmpParameter> paramListRec = TransNmpParameters(timeout, SharedDataActive.NmpChannel, EdiabasNet.IfhCommands.IfhWarmStart, paramListSend);
            if (paramListRec == null || paramListRec.Count < 4)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "*** NMT warm start failed");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }
            ...
            if (errorCode == null)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT warm start invalid parameters");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "NMT warm start result: {0}", errorCode.Value);
            return errorCode.Value;
        }

        protected EdiabasNet.ErrorCodes NmtReset()
        {
            if (SharedDataActive.ReconnectRequired)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtReset Reconnecting");
                InterfaceDisconnect(true);
                if (!InterfaceConnect(true))
                {
                    EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtReset Reconnect failed");
                    SharedDataActive.ReconnectRequired = true;
                    return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
                }
            }
            ...
            int timeout = RplusFunctionTimeout;
            List<NmpParameter> paramListRec = TransNmpParameters(timeout, SharedDataActive.NmpChannel, EdiabasNet.IfhCommands.IfhReset);
            if (paramListRec == null || paramListRec.Count < 4)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "*** NMT reset failed");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }
            ...
            if (errorCode == null)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT reset invalid parameters");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "NMT reset result: {0}", errorCode.Value);
            return errorCode.Value;
        }
        ...
        protected EdiabasNet.ErrorCodes NmtGetPowerSupply(out Int32 voltage)
        {
            voltage = 0;
            if (SharedDataActive.ReconnectRequired)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtGetPowerSupply Reconnecting");
                InterfaceDisconnect(true);
                if (!InterfaceConnect(true))
                {
                    EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtGetPowerSupply Reconnect failed");
                    SharedDataActive.ReconnectRequired = true;
                    return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
                }
            }
            ...
            if (errorCode == null)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT get power supply invalid parameters");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            if (voltageData == null || voltageData.Length != 4)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT get power supply invalid voltage data");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            voltage = (voltageData[3] << 24) + (voltageData[2] << 16) + (voltageData[1] << 8) + voltageData[0];
            EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "NMT get power supply voltage: {0}", voltage);

            EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "NMT get power supply result: {0}", errorCode.Value);
            return errorCode.Value;
        }
        ...
        protected EdiabasNet.ErrorCodes NmtGetKeyBytes(out byte[] keyBytes)
        {
            keyBytes = null;
            if (SharedDataActive.ReconnectRequired)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtGetKeyBytes Reconnecting");
                InterfaceDisconnect(true);
                if (!InterfaceConnect(true))
                {
                    EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "NmtGetKeyBytes Reconnect failed");
                    SharedDataActive.ReconnectRequired = true;
                    return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
                }
            }
            ...
            int timeout = RplusFunctionTimeout;
            List<NmpParameter> paramListRec = TransNmpParameters(timeout, SharedDataActive.NmpChannel, EdiabasNet.IfhCommands.IfhRequestKeyBytes);
            if (paramListRec == null || paramListRec.Count < 4)
            {
                EdiabasProtected?.LogString(EdiabasNet.EdLogLevel.Ifh, "*** NMT get key bytes failed");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            EdiabasNet.ErrorCodes? errorCode = paramListRec[2].GetErrorCode();
            keyBytes = paramListRec[3].GetBinary();
            if (errorCode == null)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT get key bytes invalid parameters");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }

            if (keyBytes == null)
            {
                EdiabasProtected?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "*** NMT get key bytes invalid data");
                return EdiabasNet.ErrorCodes.EDIABAS_IFH_0019;
            }
