...
{
    readerThread = new Posix_QextSerialReaderThread(this);
...
/*!
\fn void Posix_QextSerialPort::setBaudRate(unsigned int baudRate)
...
\endverbatim
*/
void Posix_QextSerialPort::setBaudRate(unsigned int baudRate)
{
    LOCK_MUTEX();
...
/*!
\fn void Posix_QextSerialPort::setBreak(bool set)
...
*/
void Posix_QextSerialPort::setBreak(bool set)
{
    LOCK_MUTEX();
    if (isOpen())
    {
        if (set)
        {
            ioctl(Posix_File->handle(), TIOCSBRK, 0);
        }
        else
        {
            ioctl(Posix_File->handle(), TIOCCBRK, 0);
        }
    }
    UNLOCK_MUTEX();
}
 ...
 */
void Posix_QextSerialPort::clearReceiveBuffer()
{
...
Posix_QextSerialReaderThread::Posix_QextSerialReaderThread(Posix_QextSerialPort* newParent)
{
    parent = newParent;
    shutdown = false;
    handle = 0;
...
Posix_QextSerialReaderThread::~Posix_QextSerialReaderThread()
{
    delete rxbuffer[0];
    delete rxbuffer[1];
}

void Posix_QextSerialReaderThread::run()
{
    int readSize = 0, lastReadSize = 0;
    int requestSize, lastRequestSize = 0;
    fd_set read_set;
    timeval timeout;
    ...
    while(shutdown == false)
    {
        ...
        if(lastReadSize == 0)
        {
            timeout.tv_usec = parent->Settings.Timeout_Millisec * 1000;
        }

        if(select(handle+1, &read_set, NULL, NULL, &timeout) >= 0)
        {
            readSize = 0;
            if(FD_ISSET(handle, &read_set))
            {
                readSize = read(handle, (void*)rxbuffer[buffer], requestSize);
            }
        }
