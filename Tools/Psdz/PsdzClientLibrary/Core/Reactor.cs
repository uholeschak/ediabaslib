...
namespace PsdzClientLibrary.Core
{
    ...
    public class Reactor
    {
        ...
        private static IMultisourceLogger log;
        ...
        public static Reactor Instance
        {
            get
            {
                if (singleton == null)
                {
                    throw new InvalidOperationException("Fusior reactor is not initialized, fire 'Initialize' before using it.");
                }
                return singleton;
            }
        }
        ...
        {
            if (!value.StartsWith("X") || source != DataSource.FBM)
            {
                AssignPropertyAndExecuteFallback(delegate
                {
