...
{
    public class XEP_ECUVARIANTS : INotifyPropertyChanged, IXepEcuVariants, IMultilanguageTitle
    {
        ...
        private string logisticsType_Field;
        ...
        public decimal? FaultMemoryDeleteWaitingTime
        {
            ...
            set
            {
                if (faultMemoryDeleteWaitingTimeField.HasValue)
                {
        ...
        public string LogisticsType
        {
            get
            {
                return logisticsType_Field;
            }
            set
            {
                if (logisticsType_Field != null)
                {
                    if (!logisticsType_Field.Equals(value))
                    {
                        logisticsType_Field = value;
                        OnPropertyChanged("LogisticsType");
                    }
                }
                else
                {
            ...
            get
            {
                ...
                switch (ConfigSettings.CurrentUICulture)
                {
                    ...
                    case "ko-KR":
                        ...
                        break;
                    default:
                        Log.Warning("XEP_ECUVARIANTS.get_Title", "CurrentUICulture {0} not available - language set to enGB", ConfigSettings.CurrentUICulture);
                        text = Title_engb;
                        break;
                }
        ...
        public virtual XEP_ECUVARIANTS Clone()
        {
            return (XEP_ECUVARIANTS)MemberwiseClone();
        }

        public XEP_ECUVARIANTS()
        {
        }

        public XEP_ECUVARIANTS(IXepEcuVariants variant)
        {
            Id = variant.Id;
            Name = variant.Name;
            FaultMemoryDeleteWaitingTime = variant.FaultMemoryDeleteWaitingTime;
            ValidFrom = variant.ValidFrom;
            ValidTo = variant.ValidTo;
            EcuGroupId = variant.EcuGroupId;
            Sort = variant.Sort;
            Sicherheitsrelevant = variant.Sicherheitsrelevant;
            ExplicitTitle = variant.Title;
            TitleId = variant.TitleId;
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (!(obj is XEP_ECUVARIANTS xEP_ECUVARIANTS))
            {
                return false;
            }
            if (Id != xEP_ECUVARIANTS.Id)
            {
                return false;
            }
            if (!(FaultMemoryDeleteWaitingTime == xEP_ECUVARIANTS.FaultMemoryDeleteWaitingTime))
            {
                return false;
            }
            if (string.CompareOrdinal(Name, xEP_ECUVARIANTS.Name) != 0)
            {
                return false;
            }
            if (!(TitleId == xEP_ECUVARIANTS.TitleId))
            {
                return false;
            }
            if (string.CompareOrdinal(Title_dede, xEP_ECUVARIANTS.Title_dede) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_engb, xEP_ECUVARIANTS.Title_engb) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_enus, xEP_ECUVARIANTS.Title_enus) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_fr, xEP_ECUVARIANTS.Title_fr) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_th, xEP_ECUVARIANTS.Title_th) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_sv, xEP_ECUVARIANTS.Title_sv) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_it, xEP_ECUVARIANTS.Title_it) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_es, xEP_ECUVARIANTS.Title_es) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_id, xEP_ECUVARIANTS.Title_id) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ko, xEP_ECUVARIANTS.Title_ko) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_el, xEP_ECUVARIANTS.Title_el) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_tr, xEP_ECUVARIANTS.Title_tr) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_zhcn, xEP_ECUVARIANTS.Title_zhcn) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ru, xEP_ECUVARIANTS.Title_ru) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_nl, xEP_ECUVARIANTS.Title_nl) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_pt, xEP_ECUVARIANTS.Title_pt) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_zhtw, xEP_ECUVARIANTS.Title_zhtw) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ja, xEP_ECUVARIANTS.Title_ja) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_cscz, xEP_ECUVARIANTS.Title_cscz) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_plpl, xEP_ECUVARIANTS.Title_plpl) != 0)
            {
                return false;
            }
            if (ValidTo != xEP_ECUVARIANTS.ValidTo)
            {
                return false;
            }
            if (ValidFrom != xEP_ECUVARIANTS.ValidFrom)
            {
                return false;
            }
            if (!(Sicherheitsrelevant == xEP_ECUVARIANTS.Sicherheitsrelevant))
            {
                return false;
            }
            if (!(EcuGroupId == xEP_ECUVARIANTS.EcuGroupId))
            {
                return false;
            }
            if (!(Sort == xEP_ECUVARIANTS.Sort))
            {
                return false;
            }
            if (string.CompareOrdinal(LogisticsType, xEP_ECUVARIANTS.LogisticsType) != 0)
            {
                return false;
            }
            return true;
