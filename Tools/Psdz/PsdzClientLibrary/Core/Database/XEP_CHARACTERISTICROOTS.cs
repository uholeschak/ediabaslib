...
{
    public class XEP_CHARACTERISTICROOTS : INotifyPropertyChanged, IXepCharacteristicRoots
    {
        ...
        private decimal? vehicleSequenceField;

        private decimal? motorCycleSequenceField;
        ...
        public decimal? VehicleSequence
        {
            get
            {
                return vehicleSequenceField;
            }
            set
            {
                if (vehicleSequenceField.HasValue)
                {
                    if (!vehicleSequenceField.Equals(value))
                    {
                        vehicleSequenceField = value;
                        OnPropertyChanged("VehicleSequence");
                    }
                }
                else
                {
        ...
        public decimal? MotorCycleSequence
        {
            get
            {
                return motorCycleSequenceField;
            }
            set
            {
                if (motorCycleSequenceField.HasValue)
                {
                    if (!motorCycleSequenceField.Equals(value))
                    {
                        motorCycleSequenceField = value;
                        OnPropertyChanged("MotorCycleSequence");
                    }
                }
                else
                {
            ...
            get
            {
                ...
                switch (ConfigSettings.CurrentUICulture)
                {
                    ...
                    case "ko-KR":
                        ...
                        break;
                    default:
                        Log.Warning("XEP_CHARACTERISTICROOTS.Title", "CurrentUICulture {0} not available - language set to enGB", ConfigSettings.CurrentUICulture);
                        text = Title_engb;
                        break;
                }
        ...
        public virtual XEP_CHARACTERISTICROOTS Clone()
        {
            return (XEP_CHARACTERISTICROOTS)MemberwiseClone();
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (!(obj is XEP_CHARACTERISTICROOTS xEP_CHARACTERISTICROOTS))
            {
                return false;
            }
            if (Id != xEP_CHARACTERISTICROOTS.Id)
            {
                return false;
            }
            if (!(Nodeclass == xEP_CHARACTERISTICROOTS.Nodeclass))
            {
                return false;
            }
            if (!(TitleId == xEP_CHARACTERISTICROOTS.TitleId))
            {
                return false;
            }
            if (string.CompareOrdinal(Title_dede, xEP_CHARACTERISTICROOTS.Title_dede) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_engb, xEP_CHARACTERISTICROOTS.Title_engb) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_enus, xEP_CHARACTERISTICROOTS.Title_enus) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_fr, xEP_CHARACTERISTICROOTS.Title_fr) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_th, xEP_CHARACTERISTICROOTS.Title_th) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_sv, xEP_CHARACTERISTICROOTS.Title_sv) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_it, xEP_CHARACTERISTICROOTS.Title_it) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_es, xEP_CHARACTERISTICROOTS.Title_es) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_id, xEP_CHARACTERISTICROOTS.Title_id) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ko, xEP_CHARACTERISTICROOTS.Title_ko) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_el, xEP_CHARACTERISTICROOTS.Title_el) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_tr, xEP_CHARACTERISTICROOTS.Title_tr) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_zhcn, xEP_CHARACTERISTICROOTS.Title_zhcn) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ru, xEP_CHARACTERISTICROOTS.Title_ru) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_nl, xEP_CHARACTERISTICROOTS.Title_nl) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_pt, xEP_CHARACTERISTICROOTS.Title_pt) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_zhtw, xEP_CHARACTERISTICROOTS.Title_zhtw) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ja, xEP_CHARACTERISTICROOTS.Title_ja) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_cscz, xEP_CHARACTERISTICROOTS.Title_cscz) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_plpl, xEP_CHARACTERISTICROOTS.Title_plpl) != 0)
            {
                return false;
            }
            if (!(MotorCycleSequence == xEP_CHARACTERISTICROOTS.MotorCycleSequence))
            {
                return false;
            }
            if (!(VehicleSequence == xEP_CHARACTERISTICROOTS.VehicleSequence))
            {
                return false;
            }
            return true;
