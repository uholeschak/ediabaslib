...
{
    public class XEP_VEHICLEPART : INotifyPropertyChanged
    {
            ...
            get
            {
                ...
                switch (ConfigSettings.CurrentUICulture)
                {
                    ...
                    case "ko-KR":
                        ...
                        break;
                    default:
                        Log.Warning("XEP_VEHICLEPART.get_Title", "CurrentUICulture {0} not available - language set to enGB", ConfigSettings.CurrentUICulture);
                        text = Title_engb;
                        break;
                }
        ...
        public virtual XEP_VEHICLEPART Clone()
        {
            return (XEP_VEHICLEPART)MemberwiseClone();
        }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (!(obj is XEP_VEHICLEPART xEP_VEHICLEPART))
            {
                return false;
            }
            if (Id != xEP_VEHICLEPART.Id)
            {
                return false;
            }
            if (!(TitleId == xEP_VEHICLEPART.TitleId))
            {
                return false;
            }
            if (string.CompareOrdinal(Title_dede, xEP_VEHICLEPART.Title_dede) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_engb, xEP_VEHICLEPART.Title_engb) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_enus, xEP_VEHICLEPART.Title_enus) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_fr, xEP_VEHICLEPART.Title_fr) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_th, xEP_VEHICLEPART.Title_th) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_sv, xEP_VEHICLEPART.Title_sv) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_it, xEP_VEHICLEPART.Title_it) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_es, xEP_VEHICLEPART.Title_es) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_id, xEP_VEHICLEPART.Title_id) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ko, xEP_VEHICLEPART.Title_ko) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_el, xEP_VEHICLEPART.Title_el) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_tr, xEP_VEHICLEPART.Title_tr) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_zhcn, xEP_VEHICLEPART.Title_zhcn) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ru, xEP_VEHICLEPART.Title_ru) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_nl, xEP_VEHICLEPART.Title_nl) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_pt, xEP_VEHICLEPART.Title_pt) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_zhtw, xEP_VEHICLEPART.Title_zhtw) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_ja, xEP_VEHICLEPART.Title_ja) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_cscz, xEP_VEHICLEPART.Title_cscz) != 0)
            {
                return false;
            }
            if (string.CompareOrdinal(Title_plpl, xEP_VEHICLEPART.Title_plpl) != 0)
            {
                return false;
            }
            return true;
