...
{
    public class XEP_FAULTCLASSES : INotifyPropertyChanged
    {
            ...
            get
            {
                ...
                switch (ConfigSettings.CurrentUICulture)
                {
                    ...
                    case "ko-KR":
                        ...
                        break;
                    default:
                        Log.Warning("XEP_FAULTCLASSES.get_Title", "CurrentUICulture {0} not available - language set to enGB", ConfigSettings.CurrentUICulture);
                        text = Title_engb;
                        break;
                }
        ...
        public override bool Equals(object obj)
        {
            ...
            if (!(obj is XEP_FAULTCLASSES xEP_FAULTCLASSES))
            {
                return false;
            }

            if (Id != xEP_FAULTCLASSES.Id)
            {
                return false;
            }

            if (!(TitleId == xEP_FAULTCLASSES.TitleId))
            {
                return false;
            }

            if (string.CompareOrdinal(Title_dede, xEP_FAULTCLASSES.Title_dede) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_engb, xEP_FAULTCLASSES.Title_engb) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_enus, xEP_FAULTCLASSES.Title_enus) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_fr, xEP_FAULTCLASSES.Title_fr) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_th, xEP_FAULTCLASSES.Title_th) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_sv, xEP_FAULTCLASSES.Title_sv) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_it, xEP_FAULTCLASSES.Title_it) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_es, xEP_FAULTCLASSES.Title_es) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_id, xEP_FAULTCLASSES.Title_id) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_ko, xEP_FAULTCLASSES.Title_ko) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_el, xEP_FAULTCLASSES.Title_el) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_tr, xEP_FAULTCLASSES.Title_tr) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_zhcn, xEP_FAULTCLASSES.Title_zhcn) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_ru, xEP_FAULTCLASSES.Title_ru) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_nl, xEP_FAULTCLASSES.Title_nl) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_pt, xEP_FAULTCLASSES.Title_pt) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_zhtw, xEP_FAULTCLASSES.Title_zhtw) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_ja, xEP_FAULTCLASSES.Title_ja) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_cscz, xEP_FAULTCLASSES.Title_cscz) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Title_plpl, xEP_FAULTCLASSES.Title_plpl) != 0)
            {
                return false;
            }

            if (string.CompareOrdinal(Regel, xEP_FAULTCLASSES.Regel) != 0)
            {
                return false;
            }

            if (!(Priority == xEP_FAULTCLASSES.Priority))
            {
                return false;
            }

            if (!(ParentId == xEP_FAULTCLASSES.ParentId))
            {
                return false;
            }
