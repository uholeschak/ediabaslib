...
namespace PsdzClient.Core
{
	...
	public class EcuTreeConfiguration : IEcuTreeConfiguration
	{
		...
		[XmlElement("EcuLogisticsList", Type = typeof(List<EcuLogisticsEntry>), IsNullable = true)]
		...
		ReadOnlyCollection<IEcuLogisticsEntry> IEcuTreeConfiguration.EcuLogisticsList
		{
			get
			{
				if (EcuLogisticsList.Count == 0)
				{
					return null;
				}
				return ((IEnumerable<IEcuLogisticsEntry>)EcuLogisticsList).ToList().AsReadOnly();
			}
		}
