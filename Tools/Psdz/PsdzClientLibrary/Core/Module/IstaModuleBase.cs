    ...
    public abstract class IstaModuleBase : IIstaModule, IDisposable
    {
        ...
        public IDiagnosticObjectLocator __DiagnosticObject(string refText)
        {
            ...
            try
            {
                ...
                //[+] ICollection<XEP_DIAGNOSISOBJECTSEX> diagObjectsByName = XepConverter.Convert(DBProvider.GetDiagObjectsByName(text, Vehicle, FFMResolver, getHidden: true));
                ICollection<XEP_DIAGNOSISOBJECTSEX> diagObjectsByName = XepConverter.Convert(DBProvider.GetDiagObjectsByName(text, Vehicle, FFMResolver, getHidden: true));
                //[+] if (diagObjectsByName != null && diagObjectsByName.Count > 0)
                if (diagObjectsByName != null && diagObjectsByName.Count > 0)
                {
                    ...
                    //[+] ICollection<XEP_DIAGNOSISOBJECTSEX> childDiagObjects = XepConverter.Convert(DBProvider.GetChildDiagObjects(diagnosticObject.GetXepDiagnosisObject(), Vehicle, FFMResolver, getHidden: true));
                    ICollection<XEP_DIAGNOSISOBJECTSEX> childDiagObjects = XepConverter.Convert(DBProvider.GetChildDiagObjects(diagnosticObject.GetXepDiagnosisObject(), Vehicle, FFMResolver, getHidden: true));
        ...
        public IEcuVariantLocator __EcuVariant(string variantName)
        {
            ...
            //[+] XEP_ECUVARIANTS ecuVariantByName = XepConverter.Convert(DBProvider.GetEcuVariantByName(variantName));
            XEP_ECUVARIANTS ecuVariantByName = XepConverter.Convert(DBProvider.GetEcuVariantByName(variantName));
        ...
        public IEcuVariantLocator __EcuVariant(decimal ecuVariantId)
        {
            ...
            //[+] XEP_ECUVARIANTS ecuVariantById = XepConverter.Convert(DBProvider.GetEcuVariantById(ecuVariantId.ToString(CultureInfo.InvariantCulture)));
            XEP_ECUVARIANTS ecuVariantById = XepConverter.Convert(DBProvider.GetEcuVariantById(ecuVariantId.ToString(CultureInfo.InvariantCulture)));
