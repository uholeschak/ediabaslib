...
namespace PsdzClient.Core
{
    public interface IInteractionService
    {
        ...
        Task<TResponse> RegisterAsync<TResponse>(InteractionRequestModel<TResponse> model)
            where TResponse : InteractionResponse;
        TResponse RegisterSync<TResponse>(InteractionRequestModel<TResponse> model)
            where TResponse : InteractionResponse;
        [PreserveSource(Hint = "IInteractionProgressModel", Placeholder = true)]
