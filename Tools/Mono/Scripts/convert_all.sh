...
"$extract_tool" "$data_dir/mandroid.exe" "mandroid"
if [ $? -ne 0 ]; then
    echo "extract failed"
    exit
fi
"$extract_tool" "$data_dir/mtouch.exe" "mtouch"
if [ $? -ne 0 ]; then
    echo "extract failed"
    exit
fi
...
cp "$script_dir/../Support/MonoTouchClient/bin/Release/Mono.Touch.Client.dll" "$data_dir/mandroid/"
cp "$script_dir/../Support/MonoTouchCommon/bin/Release/Mono.Touch.Common.dll" "$data_dir/mandroid/"
cp "$script_dir/../Support/MonoTouchCommon/bin/Release/Mono.Touch.Activation.Common.dll" "$data_dir/mtouch/"
