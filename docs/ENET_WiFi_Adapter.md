...
There is alternative commercial [Unichip ENET WiFi adapter](http://m.unichip-tec.com/bmw_enet_wifi_wireless_esys.html) available.  
...
Another solution is to use a OpenWrt WiFi router (e.g. MT7620A), update the firmware from the manufacturer and connect the cable to one of the LAN ports (not WAN) of the router.  
A buck-boost 12V DC-DC converter (e.g. DD39AJPA from eletechsup) is required for power supply.  
