...
#pragma warning restore 618
        ...
        public List<X509CertificateStructure> GenS29Certificate(EdiabasNet ediabas, AsymmetricKeyParameter machinePublicKey, List<X509CertificateStructure> trustedCaCerts, string trustedKeyPath, string certPath, string vin, out AsymmetricCipherKeyPair externalKeyPair)
        {
            ...
            try
            {
                ...
                {
                    ediabas?.LogString(EdiabasNet.EdLogLevel.Ifh, "GenS29Certificate: Load cached SubCaEmea key failed");
                    return null;
                }

                AsymmetricKeyParameter subCaKeyResource = EdBcTlsUtilities.LoadCachedKeyFile(certPath, "SubCaCert.pem", string.Empty, out X509CertificateEntry[] publicSubCaChain);
                ...
                AsymmetricKeyParameter subCaEmeaPublicKey = publicSubCaEmeaChain[0].Certificate.GetPublicKey();
        ...
        public AsymmetricKeyParameter LoadExternalCaCertificate(EdiabasNet ediabas, string certPath, List<X509CertificateStructure> trustedCaCerts, ref DateTime? certValidDate, out List<X509CertificateEntry> publicSubCaChain)
        {
            ...
            if (File.Exists(keyPwdFile))
            {
                ...
                {
                    ediabas?.LogFormat(EdiabasNet.EdLogLevel.Ifh, "LoadExternalCaCertificate: Failed to read key password file: {0}", keyPwdFile);
                    return null;
                }
            }
...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            if (isArm)
            {
                ...
                return is64Bit ? ["arm64", "arm64_v8a", "arm64-v8a", "aarch64"] : ["armeabi_v7a", "armeabi-v7a", "arm"];
            }
        ...
        + pageInfo.ClassCode;
            ...
            try
            {
                ...
                lock (CompileLock)
                {
                    ...
                    using (MemoryStream ms = new MemoryStream())
                    {
                        // ToDo: 5.6.0 not usable yet: Volatile.ReadBarrier() hangs on mono/android (interpreter).
                        // Runtime fix dotnet/runtime#124538 (.NET 11); .NET 10 backport tracked in #130265.
